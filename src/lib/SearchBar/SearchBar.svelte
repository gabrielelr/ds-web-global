<script>
  /**
   * SearchBar — Svelte 5 markup.
   *
   * Pill-shaped text input with a leading magnifying-glass icon and a
   * trailing clear button that appears when there is text to clear.
   * Visual styles live in ./SearchBar.css (imported globally via src/app.css).
   *
   * The active border is driven by `:focus-within` so the consumer just
   * binds `value` — no need to track focus manually. `state="active"` is
   * an explicit override for snapshots / Storybook.
   *
...
>
  ...
  {#if value}
    <button
      ...
      onclick={clear}
    >
      <!-- Antares close icon (Figma S8U9Li374QCzYEtwFBnaaX close). -->
      <svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
